...
; ------------------------------------------------------------------
; DIAGNOSTIC 3 phases (hook installe) :
;  phase 1 : DI + delai ~3s (pas d'interruptions, pas d'impression)
;  phase 2 : EI + delai ~3s (interruptions actives, pas d'impression)
;  phase 3 : sortie
; '1' s'affiche si phase 1 survecue, '2' si phase 2 survecue.
mainloop:	di
		call	delay3s
		ei
		ld	a,#0x31			; '1' : DI-delai survecu
		call	TXT_OUTPUT
		call	flush_to_pc

		call	delay3s			; interruptions actives, hook installe
		ld	a,#0x32			; '2' : EI-delai survecu
		call	TXT_OUTPUT
		call	flush_to_pc
		jp	quit
