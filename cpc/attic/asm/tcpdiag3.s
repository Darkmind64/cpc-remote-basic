...
wait_client:	call	esc_check
		...
		call	dispdec
		call	crlf
		; DIAGNOSTIC 3 : envoi IMMEDIAT apres accept, avant tout recv
		ld	a,#0x53		; 'S'
		call	txt_output
		ld	hl,#cmd_ok
		call	sendcmd
...
echo_loop:	call	esc_check
		...
		or	3(ix)
		jr	z, echo_loop
		; marqueur de phase : on entre dans recv
		ld	a,#0x52		; 'R'
		call	txt_output
		; demander exactement ce qui est disponible (max RECVMAX)
		ld	a, 3(ix)		; octet haut du compteur
		or	a
		jr	nz, cap_max	; >255 -> plafonner
		ld	a, 2(ix)
		cp	#RECVMAX+1
		jr	c, size_ok
cap_max:	ld	a,#RECVMAX
size_ok:	ld	(cmd_recv+4),a	; patcher la taille demandee (octet bas)
		xor	a
		ld	(cmd_recv+5),a	; octet haut = 0
		ld	hl,#cmd_recv
		call	sendcmd
		ld	a,#0x72		; 'r' : recv revenu
		call	txt_output
		ld	iy,(#resp_ptr)
		...
		pop	hl
		pop	bc
		; DIAGNOSTIC : pas d'echo (send desactive), marqueur fin d'affichage
		ld	a,#0x44		; 'D' : affichage termine
		call	txt_output
		jp	echo_loop
